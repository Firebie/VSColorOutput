...
namespace BlueOnionSoftware
{
    public static class FontAndColorStorage
    {
        ...
        {
            string str = string.Format("#{0}", color.ToString("X8"));
        ...
        public static void UpdateColors()
        {
            ...
            if (store != null)
            {
                try
                {
                    ...
                    RegistryKey regkey = VSRegistry.RegistryRoot(__VsLocalRegistryType.RegType_UserSettings, true);
                    ...
                    foreach (string key in new string[] { OutputClassificationDefinitions.FindResultsSearchTerm, OutputClassificationDefinitions.FindResultsFilename })
                    {
                        ColorableItemInfo[] value = new[] { new ColorableItemInfo() };
